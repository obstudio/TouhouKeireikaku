...
--[[
	...
	第一项可能是主公？
	第二项是该角色
	第三项至第五项，每一项都可由若干冒号分隔，分别表示忠、反、内的座次价值：
		每一项用冒号隔开的若干两位数意义为：十位数表示座次位置，是1则代表二、三号位，是2则代表非1情况下存活角色中的倒数后两位，是3则代表非1、2的情形
		个位数表示好坏：是0则代表好上天，是1则代表好，是2则代表差，是3则代表差成狗
]]
...
selectFirst = function(player, candidates) -- string
	...
	if player:getSeat() - 1 <= 2 then
		seat_place = "1"
	elseif player:getRoom():alivePlayerCount() - player:getSeat() <= 1 then
		seat_place = "3"
	else
		seat_place = "2"
	end
	...
	for _, candidate in ipairs(candidates) do
		...
		if role == "rebel" and lord:hasLordSkill("qiji$") and general:getKingdom() == "moriya" then
			value = value * 0.94
		end
		if role == "lord" and table.contains(weak_lords, general:objectName()) then
			value = value * 0.8
		end

...
function sgs.Table2SPlayerList(_table)
	local players = sgs.SPlayerList()
	for _, p in ipairs(_table) do
		players:append(p)
	end
	return players
end
...
function sgs.getDefense(player, gameProcess)
	...
	if attacker or gameProcess then
		...
		if player:hasSkill("huanni") then defense = defense + 2 end
		if player:hasSkill("feixiang") then defense = defense + 2 end
		if player:hasSkill("thbanling") then defense = defense + 3 end
	end
	...
	if player:hasSkill("maoyou") then
		...
		if player:getCards("he"):length() >= n then
			defense = defense + 3.4
		end
	end
...
function SmartAI:getUseValue(card)
	...
	if card:getTypeId() == sgs.Card_TypeEquip then
		...
		if self.player:hasSkill("jingyue") then v = v - 2 end
	...
	elseif card:getTypeId() == sgs.Card_TypeBasic then
		if card:isKindOf("Slash") then
			...
			if card:isKindOf("SoulSlash") and self.player:hasSkill("yaodao") then v = v + 4 end
			if card:getSkillName() == "yaodao" then v = v + 2 end
		...
		elseif card:isKindOf("Peach") then
			...
			if self.player:getLostHp() == 1 then
				for _, p in ipairs(self.enemies) do
					if p:hasSkill("hengong") and (not p:getWeapon() or sgs.weapon_range[p:getWeapon():getClassName()] < p:distanceTo(self.player)) then
						v = v - 2.7
					end
				end
			end
	...
	elseif card:getTypeId() == sgs.Card_TypeTrick then
		...
		if self.player:hasSkill("jingyue") then
			local x = math.min(self:getCardsNum("EquipCard", "h") + self:getCardsNum("TrickCard") + self:getCardsNum("Jink"), 3)
			v = v - 3 + x
		end
	end

	if self.player:hasFlag("RuhuanMore") and (card:isKindOf("Slash") or card:isNDTrick()) then v = v - 2 end
...
function SmartAI:adjustUsePriority(card, v)
	...
	if card:isKindOf("Slash") then
		...
		if not card:isKindOf("NormalSlash") then
			for _, p in sgs.qlist(self.room:getOtherPlayers(self.player)) do
				if p:hasSkill("huadie") then
	...
	if self:hasHuayuanSuspect() and card:getSuitString() == "club" then
		v = v - 0.32
	end
...
function SmartAI:getRuleDiscards(discard_num, min_num)
	...
	local anals = self:getCardsNum("Analeptic")
	for _, card in ipairs(cards) do
		if not self.player:isCardLimited(card, sgs.Card_MethodDiscard, true) then
			if not self.player:hasFlag("LeixuanDown") then
				table.insert(to_discard, card:getId())
			else
				if table.contains(suits, card:getSuitString()) then continue end
				if not (self:hasWeakFriend(true, false) and card:isKindOf("Peach") and peaches <= 1) and not (self:isWeak() and card:isKindOf("Jink") and jinks <= 1)
						and not ((self:isVeryWeak() or (jinks == 0 and self:isWeak())) and card:isKindOf("Analeptic") and anals <= 1) then
					table.insert(to_discard, card:getId())
					if card:isKindOf("Peach") then
						peaches = peaches - 1
					end
	...
	if #to_discard < discard_num and not self.player:isKongcheng() then
		for _, card in ipairs(cards) do
			if not table.contains(to_discard, card) and #to_discard < discard_num then
...
sgs.ai_skill_discard.gamerule = function(self, discard_num, min_num)
	...
	return self:getRuleDiscards(discard_num, min_num)
...
function SmartAI:askForNullification(trick, from, to, positive) --尼玛一把明杀 就因为自己1血就把无邪打出去了？？？？？
	...
	if self:touhouIsDamageCard(trick) then
		...
		if ("fire_attack|phoenix_flame"):match(trick:objectName()) then
	...
	if ("snatch|dismantlement|haze"):match(trick:objectName()) and to:isAllNude() then return nil end
	if trick:isKindOf("IcyFog") and to:getEquips():isEmpty() then return nil end
	...
	if (trick:isKindOf("Duel") or trick:isKindOf("FireAttack") or trick:isKindOf("AOE")
			or trick:isKindOf("PhoenixFlame")) and self:needToLoseHp(to, from) and self:isFriend(to) then
		return nil --扣减体力有利
	end
	if trick:isKindOf("Drowning") and self:needToThrowArmor(to) and self:isFriend(to) then return nil end
	if positive then
		if from and (trick:isKindOf("FireAttack") or trick:isKindOf("Duel") or trick:isKindOf("AOE") or trick:isKindOf("PhoenixFlame"))
				and (self:needDeath(to) or self:cantbeHurt(to, from)) then
			if self:isFriend(from) then return null_card end
			return
		end
		if from and trick:isKindOf("MindReading") then
			if self:isEnemy(from) and self:isVeryWeak(from) then
				return null_card
			end
			return
		end
		if ("snatch|dismantlement|haze"):match(trick:objectName()) and not to:containsTrick("YanxiaoCard") and (to:containsTrick("indulgence") or to:containsTrick("supply_shortage")) then
			if self:isEnemy(from) then return null_card end
			if self:isFriend(to) and to:isNude() then return nil end
		end
		if trick:isKindOf("IcyFog") then
			if not self:isFriend(to) then return nil end
			if (to:getArmor() and not (to:getArmor():isKindOf("SilverLion") and to:isWounded())) or (to:getDefensiveHorse())
					or (to:getPile("wooden_ox"):length() >= 2) then
				return null_card
			end
			if to:getWeapon() then
				if to:getAttackRange() > 1 and to:hasSkills(sgs.attackRange_skill) then
					return null_card
				end
				if to:getWeapon():isKindOf("Crossbow") then
					if to:getHandcardNum() >= 5 or getKnownCard(to, self.player, "Slash", true) >= 2 then
						return null_card
					end
					if from and self:isEnemy(from) and (from:getHandcardNum() >= 4 or getKnownCard(from, self.player, "Slash", true) >= 2) then
						return null_card
					end
				end
				for _, p in ipairs(self.enemies) do
					if to:inMyAttackRange(p) and to:distanceTo(p) > 1 and self:isWeak(p) then
						return null_card
					end 
				end
				if from and self:isEnemy(from) then
					if to:getAttackRange() > from:getAttackRange() and from:hasSkills(sgs.attackRange_skill) then
						return null_card
					end
					for _, p in ipairs(self.friends) do
						if not from:inMyAttackRange(p) and from:distanceTo(p) <= to:getAttackRange() and self:isWeak(p) then
							return null_card
						end
					end
				end
		...
		if from and self:isEnemy(from) and (sgs.evaluatePlayerRole(from) ~= "neutral" or sgs.isRolePredictable()) then
			...
			if self:isFriend(to) then
				if trick:isKindOf("Dismantlement") then
					...
					if ("snatch|haze"):match(trick:objectName()) then return null_card end
					if trick:isKindOf("Duel") and self:isWeak(to) then return null_card end
					if trick:isKindOf("PhoenixFlame") and self:isWeak(to) then return null_card end
			...
			elseif self:isEnemy(to) then
				...
				if ("dismantlement|snatch|haze"):match(trick:objectName()) and to:getCards("j"):length() > 0 then
					return null_card
				end
			end
		end
			...
			if trick:isKindOf("AOE") then
				...
				if self.player:objectName() == to:objectName() then
					if self:hasSkills("jieming|yiji|guixin", self.player) and
						...
						return nil
					elseif self:hasSkill("huanni") and self:hasRedHandcard()
						and (self.player:getHp() > 1 or self:getCardsNum("Peach") + self:getCardsNum("Analepic") > 0) then
						return nil
		...
		if self:isEnemy(to) and trick:isKindOf("Oracle") then
			local wizard_type, wizard = self:getFinalRetrial(self.room:getCurrent(), "oracle")
			if not self:isEnemy(wizard, to) then
				return null_card
			end
		end
		...
		if from then
			if (trick:isKindOf("FireAttack") or trick:isKindOf("Duel") or trick:isKindOf("AOE") or trick:isKindOf("PhoenixFlame"))
					and (self:needDeath(to) or self:cantbeHurt(to, from)) then
				if self:isEnemy(from) then return null_card end
				return
			end
			...
			if not (trick:isKindOf("GlobalEffect") or trick:isKindOf("AOE")) then
				if self:isFriend(from) then
					if ("snatch|dismantlement|haze"):match(trick:objectName()) and to:isNude() then
...
function SmartAI:askForCardChosen(who, flags, reason, method)
	...
	if self:isFriend(who) then
		...
		if flags:match("e") and who:getTreasure() and who:getPile("wooden_ox"):length() > 2 and (not isDiscard or self.player:canDiscard(who, who:getTreasure():getId())) then
		...
		if flags:match("e") and who:getTreasure() and who:getPile("wooden_ox"):length() > 0 and (not isDiscard or self.player:canDiscard(who, who:getTreasure():getId())) then
			return who:getTreasure():getId()
		end
		...
		if flags:match("e") and who:getTreasure() and who:getTreasure():isKindOf("Yasakaninomagatama") then
			return who:getTreasure():getId()
		end
		if flags:match("e") then
...
function sgs.ai_skill_cardask.nullfilter(self, data, pattern, target)
	...
	if self:needBear() and self.player:getHp() > 2 and self:needToLongzuan() then return "." end
...
function SmartAI:needKongcheng(player, keep)
	...
	if player:hasSkill("chongsheng") and player:getMark("chongsheng") == 0 and player:getHp() <= 2 then return true end
	if player:hasSkill("fengshi") and player:getMark("fengshi") == 0 and player:getHandcardNum() <= 3 then return true end
	if player:hasSkill("shude") and player:getPhase() == sgs.Player_Play then return true end
	if player:hasSkill("anyun") then
...
function SmartAI:hasCrossbowEffect(player)
	player = player or self.player
	return player:hasWeapon("Crossbow") or player:hasSkill("paoxiao") or player:hasFlag("CannotDuannian")
end
...
function SmartAI:getCardNeedPlayer(cards, include_self)
	...
	--Crossbow
	for _, friend in ipairs(friends) do
		if self:hasSkills("longdan|wusheng|keji|sheyue|zhusi",friend) and not self:hasSkills("paoxiao",friend)
				and friend:getHandcardNum() >=2 then
			for _, hcard in ipairs(cards) do
...
function SmartAI:hasLeastHandcards(player)
	player = player or self.player
	for _, p in sgs.qlist(self.room:getOtherPlayers(player)) do
		if p:getHandcardNum() < player:getHandcardNum() then
			return false
...
function SmartAI:isWeak(player)
	...
	if (n < 2 and player:getHandcardNum() - self:getKnownNum(player) + player:getHp() < 4) or (n < 1 and player:getHandcardNum() - self:getKnownNum(player) < 4 and player:getHp() < 3)
			or (n < 2 and player:getHp() == 1) then
		return true
	end
...
function SmartAI:canRetrial(player, to_retrial, reason)
	...
	if player:hasSkill("qiangyun") and not player:isKongcheng() then return true end
	if player:hasSkill("daorang") then
		for _, p in sgs.qlist(self.room:getAlivePlayers()) do
			if self:isFriend(player, p) and p:getCards("he"):length() > player:getHandcardNum() then
				return true
			end
		end
	end
...
function SmartAI:needRetrial(judge)
	...
	if reason == "indulgence" then
		if who:isSkipped(sgs.Player_Draw) and who:isKongcheng() then
			if (who:hasSkill("shenfen") and who:getMark("@wrath") >= 6)
				...
				or (who:hasSkill("xiongyi") and who:getMark("@arise") > 0)
				or (who:getPile("meditation"):length() > 0)
				or (who:hasSkill("herong") and who:getMark("@nuclear") > 0 and who:getHandcardNum() >= 2) then
				if self:isFriend(who) then
	...
	if reason == "oracle" then
		if self:isFriend(who) then
			if self:willSkipPlayPhase(who) and not who:isWounded() then return false end
			return judge:isGood()
		else
			return not judge:isGood()
		end
	...
	if reason == "huoyun" then
		local red
		if getKnownCard(who, self.player, "ExNihilo|heart,diamond", false) > 0 then
			if self:isFriend(who) and judge.card:isBlack() then
				return true
			end
		end
		if getKnownCard(who, self.player, "Peach|heart,diamond", false) > 0 and who:isWounded() then
			if self:isFriend(who) and judge.card:isBlack() then
				for _, p in ipairs(self.friends) do
					if p:objectName() ~= who:objectName() and p:isWounded() then
						return true
					end
				end
			end
		end
		if getKnownCard(who, self.player, "SingleTargetTrick+Slash|heart,diamond", false) >= getKnownCard(who, self.player, "SingleTargetTrick+Slash|spade,club", false) then
			red = true
		else
			red = false
		end
		if self:isFriend(who) then
			if red and judge.card:isBlack() then
				return true
			elseif not red and judge.card:isRed() then
				return true
			end
		elseif self:isEnemy(who) then
			if red and judge.card:isRed() then
				return true
			elseif not red and judge.card:isBlack() then
				return true
			end
		end
		return false
	...
	if reason == "yuni" then
		local n = judge.who:getTag("YuniAmount"):toInt()
		if judge.who:getHp() == 1 and self:isVeryWeak(judge.who) and judge.who:getHandcardNum() <= 1 then
			return judge.card:isBlack()
		end
		if judge.who:getHp() >= getBestHp(judge.who) then
			return judge.card:isRed()
		end
		if judge.who:hasSkill("lie") then
			if judge.who:isKongcheng() then
				return judge.card:isRed()
			else
				return judge.card:isBlack()
			end
		end
		if judge.who:getHandcardNum() <= judge.who:getHp() - 2 and not self:needKongcheng(judge.who) then
			return judge.card:isRed()
		end
		if n >= 3 or (n >= 2 and judge.who:getHp() > 1) then
			return judge.card:isRed()
		end
		return judge.card:isBlack()
	end
...
function SmartAI:damageIsEffective(to, nature, from)
	...
	damage = damage or sgs.DamageStruct("", from, to, 1, nature)
	...
	if to:hasSkill("cizhang") and nature == sgs.DamageStruct_Thunder then
		return false
	end
...
function countCard(card_table, class_name)
	local n = 0
...
function SmartAI:getDefenseCardsNum(flag, selfonly)
	local n = self:getCardsNum("Jink", flag, selfonly) + self:getCardsNum("Peach", flag, selfonly)
	if self:isVeryWeak() then
		n = n + self:getCardsNum("Analeptic", flag, selfonly)
	end
	return n
end
...
function SmartAI:useBasicCard(card, use)
	...
	if not (card:isKindOf("Peach") and self.player:getLostHp() > 1) and self:needBear() and self:needToLongzuan() then return end
...
function SmartAI:aoeIsEffective(card, to, source)
	...
	if to:hasArmorEffect("Vine") and not card:isKindOf("NecroMusic") then
		return false
	end
...
function SmartAI:useTrickCard(card, use)
	if not card then global_room:writeToConsole(debug.traceback()) return end
	if self.player:hasSkill("ytchengxiang") and self.player:getHandcardNum() < 8 and card:getNumber() < 7 then return end
	if self:needBear() and not ("amazing_grace|ex_nihilo|snatch|iron_chain|collateral"):match(card:objectName()) and self:needToLongzuan() then return end
...
function SmartAI:useEquipCard(card, use)
	...
	if card:isKindOf("Yasakaninomagatama") and self.player:getTreasure() and self.player:getTreasure():isKindOf("WoodenOx")
			and not self.player:getPile("wooden_ox"):isEmpty() then
		return
	end
	...
	elseif card:isKindOf("Treasure") then
		if card:isKindOf("WoodenOx") then
			...
			if self.player:getTreasure() and self.player:getTreasure():isKindOf("Yasakaninomagatama") and (not self.player:isWounded()
					or (self.player:hasArmorEffect("Vine") and (self:getCardsNum("Jink") == 0 or self:getCardsNum("Nullification") == 0))) then
				return
			end
			use.card = card
		elseif card:isKindOf("Yasakaninomagatama") then
			if getBestHp(self.player) <= self.player:getHp() then return end
			use.card = card
		end
...
function getBestHp(player)
	...
	if player:hasSkill("yuzhi") then
		return math.max(0, player:getMaxHp() - 2)
	end
	if player:hasSkill("jiahu") then
		return math.max(0, player:getMaxHp() - 1)
	end
	if player:hasSkill("citan") then
		return math.max(0, player:getMaxHp() - 3)
	end
	--[[if player:hasSkill("maoyou") then
		return math.max(0, player:getMaxHp() - 2)
	end]]--
	if player:hasSkill("jixue") and player:getMark("jixue") == 0 then
		return 1
	end
	--[[if player:hasSkill("thbanling") then
		return math.max(0, player:getMaxHp() - 2)
	end]]--
	if player:hasSkill("jinpan") then
		return math.max(0, player:getMaxHp() - 2)
	end
	--[[if player:hasSkill("chengzhao") and player:getMark("@changed") == 0 then
		return math.max(0, player:getMaxHp() - 1)
	end]]--
	if player:hasSkill("jiangqu") and player:getMark("@rotten") > 0 and player:getHp() == 2 then
		return 1
	end
	if player:hasSkill("shehun") and player:getMark("@rotten") == 0 then
		return math.max(0, player:getMaxHp() - 1)
	end
	if player:hasSkill("wangjie") then
		return math.max(0, player:getMaxHp() - 2)
	end
	if player:hasSkill("yeyu") then
		return 2
	end
...
function SmartAI:needToLoseHp(to, from, isSlash, passive, recover)
	...
	if to:hasSkill("diaoou") and not to:isKongcheng() then
		for _, p in sgs.qlist(self.room:getOtherPlayers(to)) do
			if p:getMark("@ningyou") > 0 and self:isEnemy(to, p) then
				return true
			end
		end
	end
...
function SmartAI:findPlayerToDiscard(flags, include_self, isDiscard, players, return_table)
	...
	if flags:match("h") then
		for _, enemy in ipairs(enemies) do
			...
			if #cards <= 2 and not enemy:isKongcheng() and not (enemy:hasSkills("tuntian+zaoxian") and enemy:getPhase() == sgs.Player_NotActive)
					and not (enemy:hasSkill("jisha") and getKnownCard(enemy, self.player, "Slash", false) > 0 and enemy:getMark("@sanpo") > 0) then
				for _, cc in ipairs(cards) do
...
function SmartAI:touhouDamageNature(card,from,to)
	...
	if card:isKindOf("SoulSlash") then
		nature = sgs.DamageStruct_Soul
	end
	if  card:isKindOf("FireAttack") then
		nature = sgs.DamageStruct_Fire
	end
	if card:isKindOf("PhoenixFlame") then
		nature = sgs.DamageStruct_Fire
	end
	if card:isKindOf("NecroMusic") then
		nature = sgs.DamageStruct_Soul
	end
...
function SmartAI:touhouDamageCaused(damage,from,to)
	...
	if damage.to:hasFlag("BamaoTarget") then
		damage.damage = 0
	end
	return damage
end
...
function SmartAI:touhouDamageInflicted(damage,from,to)
	...
	if to:hasSkill("shengchun") and damage.damage > 1 then
		damage.damage = 0
	end
...
function SmartAI:touhouEffectNullify(card,from,to)
	...
	if to:getPile("meditation"):length() > 0 and ("savage_assault|mind_reading|haze"):match(card:objectName()) then
		return true
	end
...
function SmartAI:touhouGetAoeValueTo(card, to, from)
	...
	if card:isKindOf("SavageAssault") then
		...
		if to:hasSkill("xinping") and to:getPile("meditation"):length() > 0 then
			value = value + 120
		end
	end
	if card:isKindOf("ArcheryAttack") then
...
function SmartAI:touhouNeedAvoidAttack(damage,from,to,ignoreDamageEffect, damageStep)
	...
	elseif real_damage.damage == to:getHp() and self:canMaoyouSelf(to) then
		return false
	else
...
function SmartAI:touhouHasLightningBenefit(player)
	if player:hasSkills("feixiang|mingyun|mingyun|jingdian|qiangyun|daorang") then
		return true
	end
	return false
end
...
function SmartAI:touhouFindPlayerToDraw(include_self, drawnum, players)
	...
	function getPlayerIndex(player, list)
		for i = 1, #list, 1 do
			if list[i]:objectName() == player:objectName() then
				return i
			end
		end
		return 0
	...
	local skills = string.split(sgs.cardneed_skill, "|")
	for i = 1, #playerlist, 1 do
		local p = playerlist[i]
		if self:isFriend(p) then --这里暂未考虑【酒】
			valuelist[i] = valuelist[i] + 3 --队友
			valuelist[i] = valuelist[i] + 2.5 - math.min(p:getHandcardNum(), 5) * 0.5 --手牌收益递减
			valuelist[i] = valuelist[i] + 4 - math.min(p:getHp(), 4) * 1 --体力收益递减
			valuelist[i] = valuelist[i] - math.min(getKnownCard(p, self.player, "Peach", true), 3) * 0.8 --明桃收益递减
			valuelist[i] = valuelist[i] - math.min(math.max(getKnownCard(p, self.player, "Jink", true) - 1, 0), 3) * 0.4 --明闪收益递减
			if self:willSkipPlayPhase(p) then
				valuelist[i] = valuelist[i] - 3
			end
			if self:willSkipDrawPhase(p) then
				valuelist[i] = valuelist[i] + 1
			end
			if self:needKongcheng(p) then
				valuelist[i] = valuelist[i] - 1.8
			end
			for _, skill in ipairs(skills) do
				if p:hasSkill(skill) then
					valuelist[i] = valuelist[i] + 1.6
				end
			end
			if self.player:hasSkill("yaoshu") then
				valuelist[i] = valuelist[i] - 1.2
			end
		elseif self:isEnemy(p) then
			valuelist[i] = valuelist[i] - 1 --对面
			if self:needKongcheng(p) and p:getHandcardNum() <= 2 and drawnum <= 1 then
				valuelist[i] = valuelist[i] + 0.4
			end
			for _, skill in ipairs(skills) do
				if p:hasSkill(skill) then
					valuelist[i] = valuelist[i] - 1
	...
	for i = 1, #playerlist, 1 do
		local p = playerlist[i]
		if valuelist[i] > math.max(0, maxvalue) then
...
function SmartAI:cantbeHurt(player, from, damageNum)
	...
	if player:hasSkill("zhuqu") and not player:isLord() and player:getHp() <= damageNum then
		if from:getHp() <= 2 and ("lord|renegade"):match(from:getRole()) then return true end
		if from:getHp() <= 2 then
			local peaches = getKnownCard(from, self.player, "Peach", true) + getKnownCard(from, self.player, "Analepic", true)
			for _, p in sgs.qlist(self.room:getAlivePlayers()) do
				if self:isFriend(p, from) and p:objectName() ~= from:objectName() then
	...
	if player:hasSkill("feixiang") then
		local friends = {}
		if player:getHp() <= damageNum and not self:canBeSaved(player, damageNum) then return false end
		for _, p in sgs.qlist(self.room:getOtherPlayers(player)) do
			if self:isFriend(player, p) and p:getHandcardNum() < p:getHp() and p:getMark("@hisou") > 0 then
				table.insert(friends, p)
			end
		end
		if player:getHandcardNum() < player:getHp() - damageNum or (player:getHp() <= damageNum and self:canBeSaved(player, damageNum))
				and p:getMark("@hisou") > 0 then
			table.insert(friends, player)
		end
		self:sort(friends, "handcard")
		for _, p1 in ipairs(friends) do
			for _, p2 in sgs.qlist(self.room:getOtherPlayers(p1)) do 
				if self:isFriend(from, p2) and p1:inMyAttackRange(p2) and (getKnownCard(p1, self.player, "Slash", true) > 0
						or p1:getHandcardNum() >= 2) then
					return true
				end
			end
		end
	end
	if player:hasSkill("fuliao") and player:getMark("@rotten") > 0 and self:isEnemy(player) then
		if player:getHp() - damageNum == 1 then
			return true
		end
		local peaches = getKnownCard(player, self.player, "Analepic", true)
		for _, p in ipairs(self.enemies) do
			peaches = peaches + getKnownCard(p, self.player, "Peach", true)
		end
		if player:getHp() - damageNum <= 0 and player:getHp() - damageNum + peaches >= 1 then
			return true
		end
	end
...
function SmartAI:canBeSaved(player)
	player = player or self.player
	if self:canMaoyouSelf(player) then return true end
	...
	if getKnownCard(player, self.player, "Peach", true) + getKnownCard(player, self.player, "Analepic", true) > 0 then
		return true
	end
	for _, p in sgs.qlist(self.room:getOtherPlayers(player)) do
		if self:isFriend(p, player) and getKnownCard(p, self.player, "Peach", true) > 0 then
			return true
		end
	end
	return false
...
function SmartAI:needDeath(player)
	...
	local maxenemymark = 0
	player = player or self.player
	if ("lord|renegade"):match(player:getRole()) then return false end
...
function SmartAI:needBear(player)
	...
	return (player:hasSkills("renjie+baiyin") and not player:hasSkill("jilve") and player:getMark("@bear") < 4)
		or (player:hasSkill("longzuan") and not has_ex)
end
--standard-ai
...
function SmartAI:getNearestEnemyDistance(player)
	player = player or self.player
	local x = 100
	for _, p in sgs.qlist(self.room:getOtherPlayers(player)) do
		if self:isEnemy(player, p) and player:distanceTo(p) < x then
			x = player:distanceTo(p)
		end
	end
	return x
end
...
function SmartAI:hasSnatchTarget()
	for _, p in sgs.qlist(room:getOtherPlayers(self.player)) do
		if self.player:distanceTo(p) <= 1 and (self:isEnemy(p) or (self:isFriend(p) and not p:getCards("j"):isEmpty())) and not p:isNude() then
			return true
		end
	end
	return false
...
function SmartAI:hasArmor(player)
	player = player or self.player
	for _, c in sgs.qlist(player:getCards("e")) do
		if c:isKindOf("Armor") then return true end
	end
	return false
end

function SmartAI:hasDiagram(player)
	player = player or self.player
	for _, c in sgs.qlist(player:getCards("e")) do
		if c:isKindOf("EightDiagram") then return true end
	end
	return false
end

function SmartAI:hasVine(player)
	player = player or self.player
	for _, c in sgs.qlist(player:getCards("e")) do
		if c:isKindOf("Vine") then return true end
	end
	return false
end

function SmartAI:hasRenwang(player)
	player = player or self.player
	for _, c in sgs.qlist(player:getCards("e")) do
		if c:isKindOf("RenwangShield") then return true end
	end
	return false
end

function SmartAI:hasLion(player)
	player = player or self.player
	for _, c in sgs.qlist(player:getCards("e")) do
		if c:isKindOf("SilverLion") then return true end
	end
	return false
end

function SmartAI:hasShortage(player)
	player = player or self.player
	for _, c in sgs.qlist(player:getCards("j")) do
		if c:isKindOf("SupplyShortage") then return true end
	end
	return false
end

function SmartAI:hasIndul(player)
	player = player or self.player
	for _, c in sgs.qlist(player:getCards("j")) do
		if c:isKindOf("Indulgence") then return true end
	end
	return false
end

function SmartAI:hasJudges(player)
	player = player or self.player
	if player:getCards("j"):length() > 0 then return true end
	return false
end

function SmartAI:hasRedCard(flags)
	flags = flags or "h"
	for _, c in sgs.qlist(self.player:getCards(flags)) do
		if c:isRed() then
			return true
		end
	end
...
function SmartAI:hasWeakFriend(include_self, is_very_weak)
	local friends
	...
	for _, p in ipairs(friends) do
		if (self:isWeak(p) and not is_very_weak) or (self:isVeryWeak(p) and is_very_weak) then
			return true
		end
	end
	return false
...
function SmartAI:hasWeakEnemy(is_very_weak)
...
function SmartAI:hasFriendWithCards(flag, min_num, include_self)
	local friends
	if include_self then
		friends = self.friends
	else
	...
	if #friends == 0 then return false end
	for _, p in ipairs(friends) do
		if p:getCards(flag):length() >= min_num then
			return true
		end
	end
	return false
...
function SmartAI:isVeryWeak(player)
	player = player or self.player
	if not self:isWeak(player) then return false end
	local n = 0
	for _, id in sgs.qlist(player:getShownHandcards()) do
		local c = sgs.Sanguosha:getCard(id)
		if c:isKindOf("Jink") then
	...
	if (n < 2 and player:getHandcardNum() - self:getKnownNum(player) + player:getHp() < 3) or (n < 1 and player:getHandcardNum() - self:getKnownNum(player) < 3 and player:getHp() < 2)
			or (n < 2 and player:getHp() == 1 and player:getHandcardNum() - self:getKnownNum(player) < 3) then
		return true
	end
	if (player:getHp() <= 1 and player:getCards("he"):length() <= 2) or (player:getHp() <= 2 and player:isKongcheng()) then return true end
	return false
end

--[[function SmartAI:isWeakerThan(player1, player2)
	if not player2 then
		player2 = player1
		player1 = self.player
	end

	if self:isWeak(player1) and not self:isWeak(player2) then return true end
	if not self:isWeak(player1) and self:isWeak(player2) then return false end
	if self:isVeryWeak(player1) and not self:isVeryWeak(player2) then return true end
	if not self:isVeryWeak(player1) and self:isVeryWeak(player2) then return false end

	local n1 = 0
	for _, id in ipairs(self:getVisibleCards(player1)) do
		local c = sgs.Sanguosha:getCard(id)
		if c:isKindOf("Jink") then
			n1 = n1 + 1
		elseif c:isKindOf("Nullification") then
	...
	local n2 = 0
	for _, id in ipairs(self:getVisibleCards(player2)) do
		local c = sgs.Sanguosha:getCard(id)
		if c:isKindOf("Jink") then
			n2 = n2 + 1
		elseif c:isKindOf("Nullification") then
	...
	local p1 = n1 * 1.05 + (player1:getHandcardNum() - self:getKnownNum(player1)) * 0.85 + player1:getHp() * 1.33
	local p2 = n2 * 1.05 + (player2:getHandcardNum() - self:getKnownNum(player2)) * 0.85 + player2:getHp() * 1.33
...
function SmartAI:isWeakerThan(player1, player2)
	...
	return sgs.getDefense(player1) < sgs.getDefense(player2)
end

function SmartAI:chooseCardToThrow(include_equip)
	if include_equip then
		assert(not self.player:isNude())
	else
		assert(not self.player:isKongcheng())
	end
	if self:needToThrowArmor() then
...
function SmartAI:hasDamageCard()
...
function SmartAI:inMyAttackRange(player1, player2, distance_fix)
	if player1:hasSkill("hengong") and (player2:getHandcardNum() >= player2:getMaxHp() or not player2:isWounded()) then
		return true
	end
	return player1:getAttackRange() >= player1:distanceTo(player2) + distance_fix
end

function SmartAI:inMyAttackRangeWithoutWeapon(player1, player2)
	if not player2 then
		player2 = player1
		player1 = self.player
	end
	...
	return player1:distanceTo(player2) <= 1
end

function SmartAI:isPriorTarget(player)
	player = player or self.player
	local friends = {}
	for _, p in sgs.qlist(self.room:getOtherPlayers(player)) do
	...
	self:sort(friends, "defense")
	if #friends > 0 and sgs.getDefense(player) <= sgs.getDefense(friends[1]) then return true end
	return false
end

function SmartAI:getKeyEquips(player)
	player = player or self.player
	key_equips = {}
	if self:isWeak(player) or self:isPriorTarget(player) then
		if player:getArmor() and not player:getArmor():isKindOf("SilverLion") and getKnownCard(player, self.player, "Jink", true) + getKnownCard(player, self.player, "Peach", true) < 2 then
			table.insert(key_equips, player:getArmor())
		end
		if player:getDefensiveHorse() and self:getAvailablePlayerNum(-1, self.enemies, player) > self:getAvailablePlayerNum(0, self.enemies, player)
				and getKnownCard(player, self.player, "Jink", true) + getKnownCard(player, self.player, "Peach", true) < 2 then
			table.insert(key_equips, player:getDefensiveHorse())
		end
	end

	if player:hasTreasure("WoodenOx") and (player:getPile("wooden_ox"):length() >= 2 or getKnownCard(player, self.player, "Peach", false, "wooden_ox") >= 1) then
		table.insert(key_equips, player:getTreasure())
	end
	...
	for _, p in ipairs(enemies) do
		if sgs.getDefense(p) < 7 then
			if player:getWeapon() and player:inMyAttackRange(p) and not self:inMyAttackRangeWithoutWeapon(player, p) then
				table.insert(key_equips, player:getWeapon())
			end
			if player:getOffensiveHorse() and player:inMyAttackRange(p) and not self:inMyAttackRange(player, p, 1) then
				table.insert(key_equips, player:getOffensiveHorse())
			end
		end
	end
	self:sortByKeepValue(key_equips)
	return key_equips
end

function SmartAI:getKeyEquipNum(player)
	player = player or self.player
	return #self:getKeyEquips(player)
end

function SmartAI:hasKeyEquip(player)
	player = player or self.player
	return self:getKeyEquipNum(player) > 0
end

function SmartAI:getOffensiveKeyEquips(player)
	local equips = {}
	player = player or self.player
	for _, c in ipairs(self:getKeyEquips()) do
		if c:isKindOf("Weapon") or c:isKindOf("OffensiveHorse") or c:isKindOf("WoodenOx") then
...
function SmartAI:hasOffensiveKeyEquip(player)
	player = player or self.player
	for _, c in ipairs(self:getKeyEquips()) do
		if c:isKindOf("Weapon") or c:isKindOf("OffensiveHorse") or c:isKindOf("WoodenOx") then
			return true
...
function SmartAI:getDefensiveKeyEquips(player)
	local equips = {}
	player = player or self.player
	for _, c in ipairs(self:getKeyEquips()) do
		if c:isKindOf("Armor") or c:isKindOf("DefensiveHorse") or c:isKindOf("WoodenOx") then
			table.insert(equips, c)
		end
	end
	return equips
...
function SmartAI:hasDefensiveKeyEquip(player)
	player = player or self.player
	for _, c in ipairs(self:getKeyEquips()) do
		if c:isKindOf("Armor") or c:isKindOf("DefensiveHorse") or c:isKindOf("WoodenOx") then
			return true
...
function SmartAI:lackCards(player)
	player = player or self.player
	return (player:objectName() ~= self.player:objectName() and player:getHandcardNum() <= 2)
			or (player:objectName() == self.player:objectName() and self:getCardsNum("Peach") + self:getCardsNum("Jink") <= 1)
end

function contains(_table, content)
	for _, cont in ipairs(_table) do
		if cont == content then
			return true
		end
	end
	return false
...
function SmartAI:isValidJiahuTarget(player)
	player = player or self.player
	for _, p in sgs.qlist(self.room:getAlivePlayers()) do
		if p:hasSkill("jiahu") and p:distanceTo(player) <= 1 and self:isFriend(p, player) then
			return true
		end
	end
	return false
...
function SmartAI:hasJiahuEffect(player)
	player = player or self.player
	return self:isValidJiahuTarget(player) and player:isWounded() and not player:getArmor()
end

function SmartAI:isHugeDamageResisted(player)
	player = player or self.player
	return self:hasLion(player) or (player:hasSkill("jinlun") and not player:getArmor())
end

function SmartAI:hasHuayuanSuspect(player)
	player = player or self.player
	for _, p in sgs.qlist(self.room:getOtherPlayers(player)) do
		if not self:isFriend(player, p) and p:hasSkill("huayuan") then
			return true
		end
	end
	return false
...
function SmartAI:getAvailablePlayers(distance_fix, suspects, player)
	player = player or self.player
	suspects = suspects or sgs.QList2Table(self.room:getOtherPlayers(player))
	local players = {}
	for _, p in ipairs(suspects) do
		if math.min(p:distanceTo(player) + distance_fix, 1) <= p:getAttackRange() then
...
function SmartAI:getAvailablePlayerNum(distance_fix, suspects, player)
	return #self:getAvailablePlayers(distance_fix, suspects, player)
end

function SmartAI:getVisibleCards(player)
	player = player or self.player
	local flag = string.format("%s_%s_%s", "visible", self.player:objectName(), player:objectName())
	if player:isKongcheng() then return {} end
	if player == self.player then
		return sgs.QList2Table(self.player:getHandcards())
	end
	local cards = {}
...
function SmartAI:allCardsSameColor()
	...
	for _, c in sgs.qlist(cards) do
		if (color == "red" and c:isBlack()) or (color == "black" and c:isRed()) then
			return false
		end
	end
	return true
...
function SmartAI:getBlackCards(include_equip)
	include_equip = include_equip or false
	local cards
	if include_equip then
...
--[[function SmartAI:findPlayerToMoveCardTo(card)
	local subtypes = {"Weapon", "Armor", "OffensiveHorse", "DefensiveHorse", "Treasure", "Indulgence", "SupplyShortage", "Lightning", "Oracle"}
	local s = ""
	for _, subtype in ipairs(subtypes) do
		if card:isKindOf(subtype) then
			s = subtype
			break
		end
	end
	if s == "" then return nil end
	if card:isKindOf("EquipCard") or card:isKindOf("Oracle") then
		self:sort(self.friends, "defense")
		for _, p in ipairs(self.friends) do
			for _, c in sgs.qlist(p:getCards("ej")) do
				if c:isKindOf(s) then continue end
			end
			return p
		end
...
function SmartAI:findCardToMoveFrom(who, response_pattern, skill_name)
	...
	if self:isFriend(who) then
		...
		if not judges:isEmpty() then
			for _, judge in sgs.qlist(judges) do
				...
				if not judge:isKindOf("YanxiaoCard") and not judge:isKindOf("Oracle") then
					for _, enemy in ipairs(self.enemies) do
			...
			if card then
				...
				for _, friend in ipairs(self.friends) do
					if not self:getSameEquip(card, friend) and friend:objectName() ~= who:objectName()
							and (self:hasSkills(sgs.need_equip_skill .. "|" .. sgs.lose_equip_skill, friend) or (skill_name == "ruhuan"
							and friend:objectName() == self.player:objectName())) then
						target = friend
						break
					end
				end
	...
	elseif not self:isFriend(who) then
		local judges = who:getJudgingArea()
		if who:containsTrick("Oracle") then
			for _, judge in sgs.qlist(judges) do
				if judge:isKindOf("Oracle") then
					card = sgs.Sanguosha:getCard(judge:getEffectiveId())
					self:sort(self.friends, "defense")
					for _, friend in ipairs(self.friends) do
		...
		if card==nil or target==nil then
			...
			if not who:getEquips():isEmpty() then
				card_id = who:getEquips():at(0):getEffectiveId()
			end]]--
			...
			if card ~= nil then
				...
				for _, friend in ipairs(self.friends) do
					if not self:getSameEquip(card, friend) and friend:objectName() ~= who:objectName()
							and (self:hasSkills(sgs.lose_equip_skill .. "|shensu" , friend) or (skill_name == "ruhuan" and friend:objectName() == who:objectName())) then
						target = friend
						break
					end
				end
	...
	if response_pattern == "card" then return card
	elseif response_pattern == "target" then return target
	else return (card ~= nil and target ~= nil) end
end

function SmartAI:notMeantToCauseEffect(card, from, to, ignore_armor)
	if self:touhouEffectNullify(card, from, to) then
		return true
	end
	if card:isKindOf("Slash") then
		if not self:slashIsEffective(card, to, from, ignore_armor) then
			return true
		end
	elseif card:isKindOf("FireAttack") or card:isKindOf("PhoenixFlame") then
		local damage = self:touhouDamage(sgs.DamageStruct(card, from, to, 1, sgs.DamageStruct_Fire), from, to)
		if damage.damage == 0 then
			return true
		end
	elseif card:isKindOf("NecroMusic") then
		local damage = self:touhouDamage(sgs.DamageStruct(card, from, to, 1, sgs.DamageStruct_Soul), from, to)
		if damage.damage == 0 then
			return true
		end
	end
...
function SmartAI:getFriendsList(player)
	player = player or self.player
	local players = sgs.SPlayerList()
	for _, p in sgs.qlist(self.room:getAlivePlayers()) do
		if self:isFriend(player, p) then
...
function SmartAI:getFriendsNoSelfList(player)
	player = player or self.player
	local players = sgs.SPlayerList()
	for _, p in sgs.qlist(self.room:getOtherPlayers(player)) do
		if self:isFriend(player, p) then
...
function SmartAI:getEnemiesList(player)
	player = player or self.player
	local players = sgs.SPlayerList()
	for _, p in sgs.qlist(self.room:getAlivePlayers()) do
		if self:isEnemy(player, p) then

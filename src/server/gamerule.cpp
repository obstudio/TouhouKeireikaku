...
//random skills
void GameRule::getRandomSkill(ServerPlayer *player, bool need_trans) const
{
	Room *room = player->getRoom();
	...
	dummy_skills << "yaofeng" << "jihan" << "xinping";
	available_wake_skills << "chongsheng";
...
bool GameRule::hasSameSkill(Room *room, QString skill_name) const
{
...
void GameRule::removeLordSkill(ServerPlayer *player) const
{
    ...
    case GameStart:
    {
        if (data.isNull()) {
            foreach (ServerPlayer *player, room->getPlayers()) {
                ...
                if ((player->getGeneral()->getKingdom() == "zhu" || player->getGeneral()->getKingdom() == "god") && player->getGeneralName() != "anjiang") {
...
void GameRule::changeGeneral1v1(ServerPlayer *player) const
{
    ...
    Q_ASSERT(player->getGeneral() != NULL);
    if (player->getGeneral()->getKingdom() == "zhu" || player->getGeneral()->getKingdom() == "god") {
        QString new_kingdom = room->askForKingdom(player);
        room->setPlayerProperty(player, "kingdom", new_kingdom);

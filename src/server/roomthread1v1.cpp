...
void RoomThread1v1::run()
{
    ...
    if (!Config.value("1v1/UsingExtension", false).toBool()) {
        const Package *stdpack = Sanguosha->findChild<const Package *>("thstandard");
        const Package *weavepack = Sanguosha->findChild<const Package *>("thweave");

        QStringList candidates;
        if (rule == "Classical") {
			foreach(const General *general, stdpack->findChildren<const General *>())
				candidates << general->objectName();
			foreach(const General *general, weavepack->findChildren<const General *>())
				candidates << general->objectName();
        }
        else {
			candidates << "reimu" << "kof_marisa" << "suika" << "aya" << "cirno"
					<< "remilia" << "flandre" << "kyouko" << "nitori"
					<< "pachouli" << "alice" << "letty" << "sanae" << "kanako"
					<< "suwako" << "satori" << "koishi" << "mystia"
					<< "yamame" << "shizuha" << "minoriko" << "tewi"
					<< "reisen" << "iku" << "yukari" << "ran" << "chen"
					<< "keine" << "mokou" << "rin" << "utsuho"
					<< "komachi" << "erin" << "kaguya";
